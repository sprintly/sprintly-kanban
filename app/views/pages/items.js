      ...
      <div className="container-tray">
        <header className="product__header container-fluid">
          ...
          <nav className="product__dropdown">
            <h1 onClick={this.toggleProductsMenu}>{this.state.product.get('name')}<span className="glyphicon glyphicon-chevron-down"/></h1>
            <ul>
            {_.map(this.state.allProducts, function(product) {
              return (
                <li><Link to="product" params={{ id: product.id }}>{product.name}</Link></li>
              )
            })}
            </ul>
          </nav>
